...
def opts_and_args():
    description = 'Postprocessing of blast csv files'
...
def filter_algo():
    ...
    with open(args.fastafile, 'r') as fasta, \
        ...
        open(root[0]+"."+str(args.recursId)+".bn_neg.fasta", 'w') \
        as neg_fasta,\
        open(root[0]+"."+str(args.recursId)+".fas", 'w') as uncovered, \
        open(root[0]+"."+str(args.recursId)+".aln", 'w') as alnfile, \
        open(root[0]+"."+str(args.recursId)+".filtered.tsv", 'w') \
            as filteredcsv:
